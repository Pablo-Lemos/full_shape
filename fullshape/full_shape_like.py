...
class FullShapeLikelihood(Likelihood):
    ...
    def initialize(self):
        ...
        # Get rid of the p4 part of the covariance
        pk_cov = pk_cov[:,:2*len(self.k_bins), :2*len(self.k_bins)]
        ...
        self.pk_data = np.concatenate([p0, p2], axis = -1)
    ...
    def logp(self, **params_values):
        ...
        self.pk_calc.sigma8 = self.provider.get_sigma8z(0)
        ...
        if self.numz > 1 and (self.redshift_dependent_bias or self.redshift_dependent_FoG): 
            ...
            for z in range(self.numz):
                ...
                if self.redshift_dependent_FoG:
                    try:
                        sigma_fog[z] = params_values['sigma_fog'+str(z+1)]
                    except: 
                        print('ERROR: Finger-of-God parameter not found')
                        print('If using multiple redshifts, you need bias parameters named sigma_fog1, sigma_fog2, sigma_fog3... in order of increasing redshift.')
                        print('Alternatively, use redshift_dependent_FoG:False and a single parameter sigma_fog1 for every bin.')
                        raise

        else:
            bias = params_values['b1']
            sigma_fog = params_values['sigma_fog1']
        ...
        pk_theory = np.concatenate(
            [self.pk_calc.p0, self.pk_calc.p2], axis = -1)
